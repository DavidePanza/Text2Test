...
def extract_content_if_needed():
    ...
    debug_log(f"TOC preview: {st.session_state['toc'][:200]}...")

    if st.session_state.toc and st.session_state.get("chapters_dict") is None:
        with st.spinner("Extracting chapters from TOC..."):
            extract_chapters_from_toc(st.session_state.toc)
            st.success("Chapters extracted successfully.")
    ...
    try:
        debug_log(f"Chapters extracted: {len(st.session_state['chapters_extracted'])}")
        debug_log(f"Preview:\n{st.session_state['chapters_extracted'][0]['content'][:1000]}")
